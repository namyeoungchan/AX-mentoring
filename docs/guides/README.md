...
2026-09-18 코드 출석 릴리스의 개정본입니다. 멘토가 발급한 코드로 학생이 Discord에서 출석하는 흐름을 반영했습니다. 각 가이드는 A4 세로 3쪽이며 본문 텍스트·아이콘은 벡터, 접속 QR은 SVG입니다. 관리자 발급 계정의 첫 로그인·이름·비밀번호 설정 절차도 포함합니다.
...
관리자는 회차 시작·코드 발급·명단 확인·마감, 멘토는 담당 조 코드 발급·만료·재발급·정정, 학생은 `/출석 코드:123456` 입력과 결과 확인을 설명합니다. 가입 인증 코드와 출석 코드를 구분하며, 기존 과제·멘토링·계정 안내도 유지합니다. 총관리자만 가능한 비밀번호 재발급·계정 삭제를 구분합니다.
