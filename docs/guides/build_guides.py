...
page('admin',2,'02 / 수업 운영','기록하고 저장한 뒤,<br/>필요한 알림을 보내세요.','출결·팀·과제의 저장 결과는 웹에서 확인합니다. Discord 알림은 별도로 전달됩니다.',
 section('ATTENDANCE','코드로 출석받고, 예외를 확인하세요')+
 flow([('회차 시작','과정 · 날짜 · 차시<br/><b>회차 시작</b>'),('코드 발급','<b>출석 코드 발급</b><br/>기본 5분 · 현장 안내'),('등록·예외 확인','학생 <b>/출석</b> 등록<br/>수기 수정은 일괄 저장'),('회차 마감','전체 미처리 0명 확인<br/><b>회차 마감</b>')])+
 note('학생 등록은 자동 저장됩니다.','명단은 5초마다 갱신됩니다. 수기 입력 중에는 갱신을 멈추므로 저장·새로고침으로 확인하세요. 코드는 1·3·5·10분 중 선택하며, 코드 종료·만료·회차 마감 후에는 등록할 수 없습니다. 마감 후 정정에는 사유가 필요합니다.')+
...
page('mentor',1,'01 / 참여와 담당 범위','담당 조를 확인하고,<br/>멘토 활동을 시작하세요.','초대받은 아이디로 계정을 만들거나 로그인한 뒤, 초대 수락을 누릅니다.',
 ...
 '<div class="role-comparison"><div><span>메인 강사</span><strong>전체 조</strong><p>회차 시작·마감 및 코드 발급</p></div><div><span>조 담당 멘토</span><strong>배정받은 조</strong><p>담당 조 코드 발급·출결 정정</p></div></div>'+
...
page('mentor',2,'02 / 수업 진행','명단을 빠르게 확인하고,<br/>수업에 집중하세요.','출결·성적·팀 배정은 본인에게 허용된 과정과 담당 조 범위에서 처리합니다.',
 ...
 flow([('회차 확인','과정 · 날짜 · 차시<br/>진행 중인지 확인'),('코드 발급','<b>출석 코드 발급</b><br/>기본 5분 · 6자리'),('학생에게 안내','수업 서버에서<br/><b>/출석 코드:123456</b>'),('예외·마감 확인','지각·결석 등 수기 저장<br/>전체 처리 후 마감')])+
 '<p class="closing">예시 123456 대신 실제 발급 코드를 안내하세요. 명단은 5초마다 갱신되며, 수기 입력 중에는 멈춥니다.</p>'+
 note('담당 조와 코드 유효시간을 확인하세요.','조 멘토의 코드는 발급 당시 담당 조의 정상 수강생만 사용합니다. 유효시간은 1·3·5·10분입니다. 재발급하면 이 회차의 내 이전 코드는 종료됩니다. 메인 강사·관리자가 회차를 시작·마감하며, 마감 후 정정에는 사유가 필요합니다.')+
...
page('mentor',3,'03 / 멘토링과 문제 해결','예약부터 피드백까지,<br/>한 흐름으로 이어가세요.','Discord 인증을 완료하고 멘토링을 받을 수 있는 시간을 먼저 등록하세요.',
 ...
 faq([
 ('코드가 만료됐거나 명단 저장이 충돌해요','만료 코드는 <b>출석 코드 재발급</b>으로 바꿔 안내하세요. 저장 충돌은 입력을 보관한 뒤 <b>명단 새로고침</b>으로 학생 등록·다른 멘토의 변경과 대조합니다.'),
...
page('student',3,'03 / 코드 출석과 나의 학습','코드로 출석하고,<br/>내 기록을 확인하세요.','해당 수업 서버에서 LMS 학생 계정의 Discord 인증을 먼저 완료하세요.',
 section('ATTENDANCE','멘토에게 받은 6자리 코드로 출석')+
 flow([('코드 확인','멘토가 안내한<br/>최신 출석 코드 확인'),('Discord 입력','수업 서버에서<br/><b>/출석 코드:123456</b>'),('등록 결과 확인','본인에게만 보이는<br/>날짜 · 차시 · 상태 확인')])+
 note('123456은 예시입니다. 실제 코드를 입력하세요.','가입 인증용 <b>/lms인증</b> 코드와 출석용 6자리 코드는 다릅니다. 코드가 만료되면 멘토에게 요청하세요. 중복 입력으로 기존 출결이 바뀌지 않으며, 정정은 멘토에게 요청합니다.')+
 section('MY LEARNING','웹에서 내 기록 확인')+
 cards(card('과정 · 팀 · 성적','상단에서 워크스페이스를 선택하고 본인 과정·팀을 확인하세요. 성적은 평가항목별 점수와 배점을 확인합니다.','users'),card('출결 · 과제 · 마감일','출결은 회차 마감 후 확정 기록이 보입니다. 과제와 마감일은 웹에서 확인하고, 제출은 Discord 패널에서 진행하세요.','check'))+
 section('QUICK HELP','막힌 단계부터 확인하세요')+
 faq([
 ('출석 등록이 안 돼요','맞는 서버·담당 조·코드 만료 여부를 확인하세요. 요청이 많다는 안내가 나오면 1분 뒤 다시 시도합니다. 이미 기록된 지각·결석은 멘토가 정정합니다.'),
 ])+
