...
page('admin',2,'02 / 수업 운영','기록하고 저장한 뒤,<br/>필요한 알림을 보내세요.','출결·팀·과제의 저장 결과는 웹에서 확인합니다. Discord 알림은 별도로 전달됩니다.',
 section('ATTENDANCE','출결: 예외를 확인하고 한 번에 저장')+
 flow([('회차 선택','과정 · 날짜 · 차시<br/><b>회차 시작</b>'),('명단 입력','출석 · 지각 · 결석 · 공결<br/>선택 인원 일괄 처리'),('저장 확인','<b>출결 일괄 저장</b><br/>저장 성공 확인'),('회차 마감','전체 미처리 0명 확인<br/><b>회차 마감</b>')])+
 note('미처리 N명 출석 버튼을 활용하세요.','현재 필터에 보이는 정상 수강생 중 미처리만 출석으로 바꿉니다. 마감 후 정정에는 사유가 필요하며, Discord 출결 공유는 저장된 집계만 전달합니다.')+
...
page('mentor',1,'01 / 참여와 담당 범위','담당 조를 확인하고,<br/>멘토 활동을 시작하세요.','초대받은 아이디로 계정을 만들거나 로그인한 뒤, 초대 수락을 누릅니다.',
 ...
 '<div class="role-comparison"><div><span>메인 강사</span><strong>전체 조</strong><p>과정 회차 시작·마감 가능</p></div><div><span>조 담당 멘토</span><strong>배정받은 조</strong><p>담당 명단의 출결 입력·정정</p></div></div>'+
...
page('mentor',2,'02 / 수업 진행','명단을 빠르게 확인하고,<br/>수업에 집중하세요.','출결·성적·팀 배정은 본인에게 허용된 과정과 담당 조 범위에서 처리합니다.',
 route('출결 화면',['멘토 활동 관리','출결 관리'])+
 flow([('회차 확인','과정 · 날짜 · 차시<br/>담당 명단 확인'),('예외 표시','지각 · 결석 · 공결<br/>해당 행에서 선택'),('나머지 출석','미처리 출석 또는<br/>선택 인원 일괄 지정'),('저장 확인','<b>출결 일괄 저장</b><br/>성공 메시지 확인')])+
 '<div class="attendance-legend"><span>✓ 출석</span><span>◷ 지각</span><span>× 결석</span><span>◇ 공결</span><span>— 미처리</span></div>'+
 note('메인 강사와 조 담당 멘토의 마감 권한은 다릅니다.','메인 강사는 회차를 시작·마감할 수 있습니다. 조 담당 멘토는 입력을 마친 뒤 마감을 요청하세요. 마감 후 변경에는 새 정정 사유가 필요합니다.')+
...
page('mentor',3,'03 / 멘토링과 문제 해결','예약부터 피드백까지,<br/>한 흐름으로 이어가세요.','Discord 인증을 완료하고 멘토링을 받을 수 있는 시간을 먼저 등록하세요.',
 ...
 faq([
 ('담당 명단이 달라졌거나 저장이 충돌해요','입력 내용을 먼저 보관하고 <b>명단 새로고침</b> 후 최신 배정을 확인하세요. 다른 멘토가 저장한 내용과 대조해 다시 입력합니다.'),
...
page('student',1,'01 / 첫 로그인과 학습 준비','받은 계정으로 로그인,<br/>가입 없이 시작하세요.','학생은 직접 가입하지 않습니다. 관리자에게 아이디와 초기 비밀번호를 받으세요.',
 ...
 note('인증은 워크스페이스마다 따로 진행합니다.','이미 다른 서버에서 인증했더라도 현재 워크스페이스의 서버에서 새로 인증하세요. 코드가 만료되면 웹에서 다시 발급받습니다.')+
 '<div class="success-strip">'+icon('check')+'<span>준비 완료 기준</span><strong>Discord 인증 완료 + 내 팀 채널 표시</strong></div>'),
page('student',2,'02 / 과제와 멘토링','알림을 확인하고,<br/>제출 완료까지 확인하세요.','과제명과 마감일은 웹의 나의 학습 또는 Discord /과제 목록에서 확인합니다.',
...
page('student',3,'03 / 나의 학습과 도움 요청','내 기록을 확인하고,<br/>궁금한 점은 담당자에게.','출결과 성적은 본인의 기록만 확인할 수 있습니다. 기록 수정은 담당 멘토에게 요청하세요.',
 section('MY LEARNING','나의 학습에서 볼 수 있는 것')+
 cards(card('과정 · 소속 팀','워크스페이스와 등록된 과정, 팀이 맞는지 확인하세요. 다른 워크스페이스는 상단 선택 메뉴로 이동합니다.','users'),card('나의 출결','관리 중인 회차는 수업 마감 후 확정된 기록이 표시됩니다. 오늘 출결이 보이지 않으면 멘토에게 마감 여부를 확인하세요.','check'),card('나의 성적','평가항목별 점수와 배점을 확인합니다. 궁금한 항목은 과정·평가명과 함께 멘토에게 문의하세요.','grid'),card('과제 · 마감일','등록 과정의 과제와 진행·마감 상태를 확인합니다. 제출은 Discord의 과제 제출 패널에서 진행합니다.','book'))+
 section('QUICK HELP','자주 막히는 세 가지')+
 faq([
 ('계정을 받았는데 채널이 보이지 않아요','첫 로그인 설정, Discord 인증, 소속 팀, 자기소개 완료를 차례로 확인하세요. 모두 완료했다면 관리자에게 문의합니다.'),
 ('인증 코드가 안 되거나 초대 링크가 만료됐어요','같은 워크스페이스의 서버인지 확인하세요. 웹에서 코드나 링크를 재발급받고 최신 것으로 진행합니다.'),
 ('로그인이 안 되거나 비밀번호를 잊었어요','아이디와 비밀번호를 확인하세요. 계속 안 되면 관리자에게 계정 복구를 요청합니다. 초기 비밀번호를 받으면 로그인 후 변경하세요.'),
 ])+
 '<div class="help-path"><span>학습·출결·과제 문의</span><b>담당 멘토</b><i>→</i><span>계정·팀·서버 문의</span><b>워크스페이스 관리자</b></div>'+
 note('문의할 때 이렇게 전달하세요.','워크스페이스 이름 · 과제/회차 · 오류가 난 시각 · 오류 문구를 알려주세요. 비밀번호나 인증 코드는 공유하지 마세요.')),
]
styles=(HERE/'guide.css').read_text()
