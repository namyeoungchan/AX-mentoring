...
def fit_embeds(embeds, marker):
    ...
    for source in embeds:
        ...
        embed.set_footer(text=marker + (f" · {embed.footer.text}" if embed.footer.text and not embed.footer.text.startswith("AX LearningOps") else ""))
    ...
    if target is None:
        for source in [channel.pins(limit=100), channel.history(limit=100)]:
            async for candidate in source:
                if candidate.author.id == channel.guild.me.id and any(e.footer.text == legacy_marker or (e.footer.text or "").startswith(marker) for e in candidate.embeds):
                    target = candidate
                    break
            if target:
                break
