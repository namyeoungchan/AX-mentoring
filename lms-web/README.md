...
멘토 코드 발급·학생 `/출석` 등록: [ATTENDANCE-CODES.md](./ATTENDANCE-CODES.md).
...
| 출결 | 멘토 코드 발급·Discord `/출석`, 회차 시작·마감, 담당 범위 명단 일괄 입력, 미처리 인원, 사후 정정 사유·전후 이력, 확정된 본인 기록 조회 | GPS 위치 검증, 자동 지각·결석 판정 및 알림 |
