...
| 작업 | 현재 경로와 조건 |
| --- | --- |
| Discord 서버 생성 | 관리자가 Discord에서 서버 생성 후 채널 링크 복사 (서버 ID도 지원) |
| 웹에 서버 등록 | 워크스페이스의 **Discord 빠른 설정 → Discord 채널 링크 또는 서버 ID → 서버 연결하고 계속** |
| 봇 초대 시작 | 다음 단계의 **이 서버에 앱 초대**. 봇 ID가 heartbeat로 확인된 뒤 표시 |
...
현재 `src/DiscordQuickSetup.tsx`와 상세 설정 `src/DiscordSetup.tsx`는 관측된 봇 ID와 연결된 서버 ID로 OAuth 설치 링크를 만들며, 서버 선택을 고정합니다. `server/provision.mjs`는 봇 heartbeat와 작업 상태를 저장합니다. 설치 링크를 열었다는 사실만으로 구축 성공을 처리하지 않습니다.
...
운영자가 만드는 서버는 Discord에서 준비하고 웹에 연결하는 절차를 유지합니다. 서버 생성 버튼 위치는 [Discord 서버 만들기 안내](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server)를 참고하세요. 현재 공식 Guild API 문서에는 일반 서버 생성 엔드포인트가 나열되어 있지 않으므로 웹의 무인 서버 생성 기능을 제공한다고 안내하지 않습니다. [Guild API](https://docs.discord.com/developers/resources/guild)

## 현장 확인 순서

1. Discord에서 대상 서버를 만들고 서버 관리 권한을 확인합니다.
2. 웹 빠른 설정에서 조 구성을 저장하고 Discord 채널 링크 또는 서버 ID를 등록합니다. 채널은 저장된 기본 구성을 사용하며 상세 편집은 선택 사항입니다.
3. 봇이 온라인인데 초대 버튼이 없다면 heartbeat/API 연결을 확인합니다.
4. **이 서버에 앱 초대**를 눌러 Discord에서 승인합니다. Private Bot 설정이면 앱 소유자만 초대할 수 있으므로 운영자와 확인합니다.
5. 연결·구축 결과가 자동 표시됩니다. 실패 시 권한·역할 순서를 확인하고 구축 다시 시도를 누릅니다.
6. 내 LMS 인증에서 코드를 복사하고 시작하기 채널로 이동해 인증합니다. 완료 후 웹 상태가 자동 갱신됩니다.
