...
export default defineConfig({
  ...
  ].map(server=>server.command==='node server/index.mjs' ? {...server,env:{...server.env,VIMEO_ACCESS_TOKEN:'',VIMEO_EMBED_DOMAINS:'',DATABASE_URL:process.env.PLAYWRIGHT_DATABASE_URL||'',PG_NAMESPACE:'e'+randomUUID().replaceAll('-','').slice(0,12),PG_POOL_MAX:'6'}} : server),
})
