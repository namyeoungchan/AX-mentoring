...
type State = { worker?: { id: string }; template: Template | null; boundGuildIds: string[]; enabled: boolean; plans: Plan[]; guilds: { id: string; name: string; connected: boolean; manageChannels: number; seenAt: number }[]; jobs: { id: string; guildId: string; state: string; errorCode: string | null; createdAt: number; results: { id: string; discordId: string; action: string }[] }[] }
...
export default function DiscordSetup({ workspaceId }: { workspaceId: string }) {
  ...
  return <>
    ...
    <GroupSetup key={workspaceId} workspaceId={workspaceId} /><section className="panel discord-server-registration"><CardHeading title="2. Discord 서버 연결 및 봇 초대" subtitle="Discord에서 서버를 만들고 개발자 모드를 켜 서버 ID를 복사하세요. 워크스페이스마다 서버 하나를 연결합니다." /><form className="modal-form" onSubmit={addServer}><fieldset disabled={busy || demoMode || state.boundGuildIds.length > 0 || !state.template || (!draft.guildId && dirty)}><label>추가할 Discord 서버 ID<input name="guildId" required inputMode="numeric" pattern="[0-9]{17,20}" maxLength={20} placeholder="서버 ID 17~20자리" /></label><button className="button primary" type="submit"><Plus size={15} />서버 추가 및 구축</button></fieldset>{!draft.guildId && dirty && <p className="discord-hint">변경한 기본 구성을 먼저 저장하세요.</p>}<p className="discord-hint">구축 시 온보딩 대기·수강생·강사·운영자·온보딩 완료 역할을 함께 생성합니다. 같은 Render 봇이 이미 참여한 서버는 다음 작업 확인 때 구축합니다. 미참여 서버는 봇을 초대한 뒤 처리합니다.</p></form>{state.boundGuildIds.length > 0 && <p className="discord-hint">연결된 서버: {state.boundGuildIds.join(' · ')}{state.worker?.id && <a className="button primary" target="_blank" rel="noreferrer" href={`https://discord.com/oauth2/authorize?client_id=${state.worker.id}&scope=bot%20applications.commands&permissions=2251800216456273&guild_id=${state.boundGuildIds[0]}&disable_guild_select=true`}>이 서버에 봇 초대</a>}</p>}</section>
    <div className="discord-setup-grid"><section className="panel discord-editor"><CardHeading title={draft.guildId ? "서버별 채널 구성" : "워크스페이스 기본 구성"} subtitle="기본 구성은 새로 추가하는 서버에 적용됩니다." />
      ...
      </fieldset><p className="discord-hint">같은 이름·유형·카테고리의 채널은 재사용합니다. 과제-대시보드는 강사·운영자 전용 권한을 적용합니다. 그 외 기존 채널은 삭제하거나 이동하지 않습니다.</p>
