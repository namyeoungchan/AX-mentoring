...
export default function DiscordSetup({ workspaceId }: { workspaceId: string }) {
  ...
  return <>
    ...
    <section className="panel discord-server-registration"><CardHeading title="Discord 서버 추가" subtitle="저장한 기본 구성으로 카테고리·텍스트·음성 채널을 자동 구축합니다." /><form className="modal-form" onSubmit={addServer}><fieldset disabled={busy || demoMode || !state.template || (!draft.guildId && dirty)}><label>추가할 Discord 서버 ID<input name="guildId" required inputMode="numeric" pattern="[0-9]{17,20}" maxLength={20} placeholder="서버 ID 17~20자리" /></label><button className="button primary" type="submit"><Plus size={15} />서버 추가 및 구축</button></fieldset>{!draft.guildId && dirty && <p className="discord-hint">변경한 기본 구성을 먼저 저장하세요.</p>}<p className="discord-hint">같은 Render 봇이 이미 참여한 서버는 다음 작업 확인 때 구축합니다. 미참여 서버는 봇을 초대한 뒤 처리합니다.</p></form>{state.boundGuildIds.length > 0 && <p className="discord-hint">연결된 서버: {state.boundGuildIds.join(' · ')}</p>}</section>
