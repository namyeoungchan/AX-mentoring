...
export function ModalShell({ title, children, close, className = '', busy = false }: { title: string; children: ReactNode; close: () => void; className?: string; busy?: boolean }) {
  const dialog = useRef<HTMLDialogElement>(null)
  useEffect(() => { const node = dialog.current; node?.showModal(); node?.querySelector<HTMLElement>('[data-modal-autofocus]')?.focus(); return () => node?.close() }, [])
  return <dialog ref={dialog} className={`modal ${className}`} onCancel={e => { e.preventDefault(); if (!busy) close() }} onClick={e => { if (!busy && e.target === e.currentTarget) close() }} aria-label={title} aria-busy={busy || undefined}><div className="modal-heading"><div><span className="eyebrow">LEARNINGOPS</span><h2>{title}</h2></div><button className="icon-button" onClick={close} disabled={busy} aria-label="닫기"><X size={20} /></button></div>{children}</dialog>
}
