...
type Account = { id: string; username: string; name: string; role: string; isSuperAdmin?: boolean; canResetPassword?: boolean; mustChangePassword: boolean; canDelete: boolean; memberships: { workspaceId: string; role: string; name: string }[] }
const roles: Record<string, string> = { admin: '관리자', instructor: '강사', student: '수강생' }
export default function AccountAdministration({ workspaceId, workspaceName, currentId, logout }: { workspaceId: string; workspaceName: string; currentId: string; logout: () => Promise<void> }) {
  ...
  return <section className="panel account-admin-panel">
    <CardHeading title="전체 계정 관리" subtitle={workspaceId ? `총관리자 전용 · 역할 변경은 현재 워크스페이스 ‘${workspaceName}’에 적용됩니다.` : '총관리자 전용 · 역할을 변경하려면 워크스페이스를 먼저 선택하세요.'} />
    ...
    {loading ? <p className="inline-note">계정을 불러오는 중…</p> : <div className="table-scroll"><table><thead><tr><th>이름 · 아이디</th><th>권한 · 소속</th><th>비밀번호</th><th>워크스페이스 역할</th><th>관리</th></tr></thead><tbody>{filtered.map(account => <tr key={account.id}>
      <td><strong>{account.name}</strong><div>{account.username}{account.id === currentId && ' · 내 계정'}</div></td>
      <td>{account.isSuperAdmin ? '슈퍼관리자' : account.role === 'admin' ? '총관리자' : account.memberships.length ? account.memberships.map((m, i) => <div key={i}>{m.name} · {roles[m.role]}</div>) : '소속 없음'}</td>
      <td>{account.mustChangePassword ? '초기 비밀번호 변경 대기' : '본인 비밀번호 사용'}</td>
      <td><button className="button secondary" disabled={busy || !workspaceId || account.role === 'admin' || account.isSuperAdmin} title={account.role === 'admin' ? '총관리자·슈퍼계정은 별도 테스트 계정을 사용하세요.' : undefined} onClick={() => { setRoleAccount(account); setError(''); setNotice('') }}>역할 변경</button><small>{account.role === 'admin' ? '전체 관리자 권한 유지' : roles[account.memberships.find(m => m.workspaceId === workspaceId)?.role || ''] || '현재 워크스페이스 소속 없음'}</small></td>
      <td><div className="account-admin-actions"><button className="button secondary" disabled={busy || account.canResetPassword === false} onClick={() => { setAction({ account, kind: 'reset' }); setConfirmation(''); setInitialPassword(''); setCopied(false); setError('') }}>초기 비밀번호 재설정</button><button className="button danger" disabled={busy || !account.canDelete} title={account.isSuperAdmin ? '슈퍼계정은 웹에서 삭제할 수 없습니다.' : !account.canDelete ? '마지막 총관리자는 삭제할 수 없습니다.' : undefined} onClick={() => { setAction({ account, kind: 'delete' }); setConfirmation(''); setError('') }}>계정 삭제</button></div>{!account.canDelete && <small>{account.isSuperAdmin ? '슈퍼계정 보호' : '마지막 총관리자 보호'}</small>}</td>
    </tr>)}</tbody></table>{!filtered.length && <p className="calendar-empty">표시할 계정이 없습니다.</p>}</div>}
    {roleAccount && <AccountRoleEditor workspaceId={workspaceId} account={roleAccount} close={() => setRoleAccount(null)} saved={() => { setRoleAccount(null); setReload(n => n + 1); setNotice('역할을 변경했습니다. 대상 계정으로 다시 로그인하면 새 역할이 적용됩니다.') }} />}
