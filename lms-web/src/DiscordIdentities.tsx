...
type Identity = { discordId: string; account: { id: string; username: string; name: string } | null; records: Record[]; state: string; orphanCount: number; revision: string; canUnlink?: boolean }
...
export default function DiscordIdentities({ logout, workspaceId, workspaceName }: { logout: () => Promise<void>; workspaceId?: string; workspaceName?: string }) {
  const endpoint = workspaceId ? `workspaces/${encodeURIComponent(workspaceId)}/discord-identities` : 'admin/discord-identities'
  const unlinks = (row: Identity) => Boolean(row.account && (!workspaceId || row.canUnlink))
  ...
  async function remove() {
    ...
    try {
      const result = await apiRequest(`${endpoint}/${selected.discordId}`, { method: 'DELETE', body: JSON.stringify({ discordId: confirmation, revision: selected.revision }) })
      setNotice(`${selected.discordId} 인증 기록을 정리했습니다. ${workspaceId && !unlinks(selected) ? '선택한 워크스페이스의 기록만 정리했습니다. 계정의 전체 연결 해제가 필요하면 총관리자에게 요청하세요.' : '웹에서 새 코드를 발급받아 Discord 인증을 진행하세요.'}`)
  ...
  return <section className="identity-panel" aria-label="Discord 인증 기록 관리">
    <div className="identity-intro"><ShieldCheck size={22} /><p>{workspaceId ? `${workspaceName || '선택한 워크스페이스'}의 Discord 연결과 인증 기록입니다.` : '전체 워크스페이스의 Discord 연결과 인증 기록입니다.'} <strong>고아 기록</strong>은 계정이 없거나 현재 연결과 맞지 않는 기록입니다.</p></div>
    ...
    {selected && <ModalShell title={unlinks(selected) ? 'Discord 인증 연결 해제' : 'Discord 인증 기록 삭제'} busy={busy} close={() => { if (!busy) { setSelected(null); setError('') } }}><form className="modal-form" onSubmit={event => { event.preventDefault(); void remove() }}>
      <p><strong>{selected.discordId}</strong><br />{selected.account ? `${selected.account.name} · ${selected.account.username}` : '연결된 LMS 계정 없음'}</p>
      <p>{workspaceId ? `${workspaceName || '선택한 워크스페이스'}의 서버 인증과 인증 코드 이력을 삭제합니다.` : '이 Discord ID의 전체 서버 인증과 인증 코드 이력을 삭제합니다.'}{unlinks(selected) && ' 현재 LMS 계정의 Discord 연결을 해제하고 해당 계정의 로그인을 모두 종료합니다.'} 정리 후에는 웹에서 새 인증 코드를 발급받으세요.</p>
      {workspaceId && !unlinks(selected) && <p>계정의 전체 Discord 연결은 유지됩니다. 다른 워크스페이스에서도 사용하는 계정이나 총관리자 계정의 연결 해제는 총관리자에게 요청하세요.</p>}
