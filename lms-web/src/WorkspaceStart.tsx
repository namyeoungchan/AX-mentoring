...
export default function WorkspaceStart({ platformAdmin, go }: { platformAdmin: boolean; go: (page: string) => void }) {
  const steps = [
    { title: '운영자 초대', description: '워크스페이스를 함께 운영할 관리자를 지정해요.', owner: '총관리자', icon: ShieldCheck, tone: 'sage', action: platformAdmin ? '관리자 초대' : '', page: 'members', note: '총관리자가 진행해요' },
    { title: '학습 공간 구축', description: '조를 구성하고 Discord 서버에 봇을 연결해요.', owner: '워크스페이스 관리자', icon: Server, tone: 'sky', action: 'Discord 빠른 설정', page: 'discord', note: '' },
  ...
  return <section className="workspace-start" aria-labelledby="workspace-start-title">
    ...
    <details className="workspace-setup-guide"><summary>관리자 설정 순서 · 화면과 버튼 안내</summary>
      ...
      <ol>
        <li><h3>관리자 참여</h3><p>총관리자: <strong>구성원 · 초대 → 참여 권한: 워크스페이스 관리자 → 초대 링크 만들기</strong>. 초대받은 관리자는 안내된 계정으로 로그인하고 <strong>초대 수락</strong>을 누릅니다.</p>{platformAdmin && <button className="button secondary" onClick={() => go('members')}>관리자 초대 화면 열기</button>}</li>
        ...
        <li><h3>학생 계정 발급과 수업 준비</h3><p><strong>학생 계정 발급</strong>에서 로그인 아이디와 과정·조를 지정하고 초기 비밀번호를 발급하세요. 학생에게 계정 정보를 전달하면 첫 로그인에서 이름과 새 비밀번호를 설정합니다. 수강생의 Discord 참여·인증 뒤 <strong>수강생 관리</strong>에서 과정과 팀을 확인합니다. 멘토에게는 첫 로그인 후 기본 정보·Discord 인증·업무 안내를 완료하도록 안내하세요.</p><button className="button secondary" onClick={() => go('student-accounts')}>학생 계정 발급 화면 열기</button></li>
      </ol>
