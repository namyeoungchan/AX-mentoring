...
type Data = { revision: string; teams: { id: string; name: string; courseId: string; courseTitle: string }[]; guildIds: string[]; archived: boolean; accounts: { id: string; username: string; name: string; setupPending: number; passwordPending: number; teamId: string; courseId: string; state: string }[] }
...
export default function StudentAccounts({ workspaceId, refreshWorkspace }: { workspaceId: string; refreshWorkspace?: () => Promise<void> }) {
  ...
  return <section className="panel student-account-panel">
    <CardHeading title="학생 계정 발급" subtitle="계정 발급 · 학생 목록에서 인증 전후 조 변경 · 학생은 직접 가입하지 않습니다." />
