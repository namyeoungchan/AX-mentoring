...
test('channel template saves, bot join applies, failures are visible and retry succeeds', async ({ page, request }) => {
  ...
  await expect(page.getByLabel('추가할 Discord 서버 ID')).toBeDisabled()
  const stateBefore = await (await page.request.get('/api/workspaces/default/discord/provision')).json()
  const duplicate = await page.request.post('/api/workspaces/default/discord/provision/servers', { data: { guildId: '788456789012345679', templateRevision: stateBefore.template.revision } })
  expect(duplicate.status()).toBe(409)
  const stateAfter = await (await page.request.get('/api/workspaces/default/discord/provision')).json()
  expect(stateAfter.boundGuildIds).toEqual([guildId])
