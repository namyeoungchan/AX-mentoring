...
관리자·강사 초대 링크는 지정한 아이디로만 수락할 수 있습니다. 7일간 유효하고 한 번만 사용되며 취소할 수 있습니다. DB에는 링크 토큰의 해시만 저장합니다. 링크는 생성 직후 복사해서 직접 전달합니다. 이메일이나 Discord DM을 자동 발송하지 않습니다. 초대받은 관리자·강사는 이름·아이디·비밀번호로 가입하고 봇의 일회용 코드 인증으로 Discord 계정을 연결한 뒤 개인 계정으로 로그인하고 초대를 수락합니다.
...
구축 시 `LMS 온보딩 대기`, `LMS 수강생`, `LMS 강사`, `LMS 운영자`, `LMS 온보딩 완료` 역할을 함께 생성하고 서버별 ID를 저장합니다. 재시도 시 저장한 역할을 재사용하며, 이름이 같은 기존 관리자 역할은 임의로 가져오지 않습니다. 봇 역할이 관리 대상 역할보다 위에 있어야 합니다. 역할 생성이나 ID 저장에 실패하면 구축 완료로 표시하지 않습니다. 역할 자동 부여와 팀 채널 권한은 **온보딩 · 팀 연동**에서 켠 경우에만 적용합니다.
...
- 봇은 서버 초대 이벤트와 30초 주기로 작업을 확인합니다. 이 기능은 봇이 참여한 여러 서버를 지원합니다. 수강생 인증은 승인된 워크스페이스의 연결 서버를 사용하고 관리자·강사 가입 인증은 초대 링크의 워크스페이스에 연결된 서버를 사용합니다.
...
최초 전체 관리자는 설정 키로 등록하며, 초대받은 관리자·강사는 초대 대상 워크스페이스의 서버에서 인증합니다. 수강생은 가입 승인 워크스페이스의 서버에서 인증합니다. 봇 초대에 `bot`·`applications.commands` scope를 포함하세요. 봇은 참여 서버마다 운영 명령과 `/lms인증`을 동기화합니다. `GUILD_ID`와 `LEARNINGOPS_AUTH_GUILD_ID`는 신규 설정에 필요하지 않습니다. 이전 초대 없는 가입 API를 사용하는 배포만 `LEARNINGOPS_AUTH_GUILD_ID`를 호환용으로 유지할 수 있습니다. 수강생 초대 발급과 웹 저장소 연결에는 `LEARNINGOPS_PROVISION_URL/TOKEN`을 사용합니다.
