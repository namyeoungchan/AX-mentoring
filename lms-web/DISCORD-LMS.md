...
워크스페이스를 만들 때 서버 ID를 입력해도 기본 구성과 구축 작업이 함께 생성됩니다. 같은 ID를 다시 등록하면 기존 설정·작업을 반환하며 중복 구축하지 않습니다. 다른 워크스페이스에 연결된 ID는 거절합니다. 봇 연결 키가 아직 없어도 대기 작업은 보존됩니다.
...
이 명령은 Git에서 제외된 `lms-web/.env`에 최초 관리자 설정 키·가입 인증 키·채널 설정 키를 생성합니다. 기존 값은 유지하고 값을 터미널에 출력하지 않습니다. 최초 관리자 등록 시 이 파일의 `ADMIN_PASSWORD`를 설정 키로 입력한 뒤 개인 아이디·비밀번호로 로그인합니다. `LEARNINGOPS_AUTH_GUILD_ID`에는 실제 교육 서버 ID를 입력하세요.
