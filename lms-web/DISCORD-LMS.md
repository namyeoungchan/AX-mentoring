...
1. 회원가입 화면에서 워크스페이스·이름·아이디·비밀번호를 입력합니다. 강사·수강생은 Discord 사용자 ID를 입력하지 않습니다. 이 단계에서 교육 Discord 서버에 참여할 필요는 없습니다.
...
6. 봇에서 인증한 실제 Discord 계정이 자동 연결되고 승인된 서버가 일치하면 워크스페이스 참여가 완료됩니다. **인증 후 학습 화면 열기**를 누르면 본인 학습 화면으로 이동합니다.
