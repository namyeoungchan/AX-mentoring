...
async function fixture(t, overrides = {}) {
    ...
    let runtime = await createRuntime(dbPath, env), failOpen = false;
    const maintenance = createDataMaintenance({ dbPath, env: { ...env, ...overrides }, getRuntime: () => runtime,
        closeRuntime: () => runtime.close(), openRuntime: async () => {
            if (failOpen) {
                failOpen = false;
                throw new Error('Injected reopen failure');
            }
            runtime = await createRuntime(dbPath, env);
        } });
...
test('backup/reset/restore covers archived workspaces, credentials, sequences, WAL and automatic recovery downloads', async (t) => {
    ...
    await f.runtime.workspaces.setArchived(extra.id, true, f.login.user);
    const db = (await f.runtime.workspaces.open(extra.id)).db;
    await (db.prepare("INSERT INTO mentors(discord_id,name) VALUES('123456789012345679','보관 멘토')")).run();
    await (db.prepare("INSERT INTO lms_attendance_codes VALUES('code','hash','course','2026-09-18',1,'guild','actor','[]',0,9999999999999,NULL)")).run();
    await (db.prepare("INSERT INTO lms_outbox(id,event_key,kind,source_id,payload,actor,created_at,state) VALUES('pending','pending','notice','n','{}','admin',0,'pending'),('sent','sent','notice','s','{}','admin',0,'sent'),('manual','manual','notice','m','{}','admin',0,'manual')")).run();
    const bytes = await m.exclusive(async () => await m.backup());
    assert.equal(decodeBackup(bytes).databases.length, 3);
    const reset = await m.exclusive(async () => await m.replace('reset', null, f.login.user.id));
    assert.equal((await f.runtime.store.db.prepare('SELECT COUNT(*) n FROM lms_users').get()).n, 0);
    assert.equal((await f.runtime.store.db.prepare('SELECT COUNT(*) n FROM lms_workspace_guilds').get()).n, 0);
    assert.equal(await f.runtime.auth.setupEnabled(), true);
    assert.equal(await f.runtime.auth.session(f.login.token), null);
    ...
    const preview = await m.exclusive(async () => await m.preview(bytes, fresh.user.id));
    assert.equal(preview.accounts, 1);
    assert.equal(preview.workspaces, 3);
    await assert.rejects(m.exclusive(async () => await m.replace('restore', preview.token, 'someone-else')), /다시 선택/);
    await m.exclusive(async () => await m.replace('restore', preview.token, fresh.user.id));
    const restored = (await f.runtime.workspaces.open(extra.id)).db;
    assert.equal((await (restored.prepare('SELECT name FROM mentors')).get()).name, '보관 멘토');
    assert.equal((await (restored.prepare('SELECT expires_at FROM lms_attendance_codes')).get()).expires_at, 0);
    assert.equal((await f.runtime.workspaces.metadata(extra.id)).archivedAt !== null, true);
    assert.equal(await f.runtime.auth.session(f.login.token), null);
    assert.equal(await f.runtime.auth.session(fresh.token), null);
    assert.equal((await f.runtime.auth.login({ username: 'data.admin', password: 'admin-password-12345' })).user.id, f.login.user.id);
    assert.deepEqual((await (restored.prepare('SELECT id,state FROM lms_outbox ORDER BY id')).all()).map(r => [r.id, r.state]), [['manual', 'manual'], ['pending', 'held'], ['sent', 'sent']]);
    assert.equal(m.backups().length, 2);
    assert.equal((await (restored.prepare("INSERT INTO mentors(discord_id,name) VALUES('123456789012345680','다음 멘토') RETURNING id")).get()).id, 2);
});
test('malformed, tampered, duplicate, path traversal, SQL identifiers and broken relationships never change live data', async (t) => {
    const f = await fixture(t), m = f.maintenance, bytes = await m.backup();
    for (const bad of [Buffer.from('not a backup'), bytes.subarray(0, bytes.length - 10),
        ...
        await assert.rejects(async () => await m.preview(bad, f.login.user.id), /백업 파일/);
    const envelope = JSON.parse(gunzipSync(bytes));
    envelope.payload += ' ';
    await assert.rejects(async () => await m.preview(gzipSync(JSON.stringify(envelope)), f.login.user.id), /백업 파일/);
...
test('a failed runtime reopen rolls every database back and keeps a usable safety backup', async (t) => {
    const f = await fixture(t), m = f.maintenance;
    await ((await f.runtime.workspaces.open('asan-ax')).db.prepare("INSERT INTO mentors(discord_id,name) VALUES('123456789012345679','원본')")).run();
    f.failNextOpen();
    await assert.rejects(m.exclusive(async () => await m.replace('reset', null, f.login.user.id)), /Injected/);
    assert.equal((await f.runtime.auth.session(f.login.token)).id, f.login.user.id);
    assert.equal((await ((await f.runtime.workspaces.open('asan-ax')).db.prepare('SELECT name FROM mentors')).get()).name, '원본');
    assert.equal(m.backups().length, 1);
...
test('reset refuses missing bootstrap key; destructive reauthentication requires a real platform admin', async (t) => {
    ...
    await assert.rejects(f.maintenance.exclusive(async () => await f.maintenance.replace('reset', null, f.login.user.id)), /설정 키/);
...
test('backup validation preserves submission rows without executing notification triggers', async (t) => {
    const f = await fixture(t), db = (await f.runtime.workspaces.open('asan-ax')).db;
    await (db.prepare("INSERT INTO assignments(id,week,title,due_date) VALUES(1,1,'과제','2030-01-01')")).run();
    await (db.prepare("INSERT INTO submissions(assignment_id,user_id,user_name,content) VALUES(1,'student','학생','제출')")).run();
    const count = (await (db.prepare('SELECT COUNT(*) n FROM lms_outbox')).get()).n;
    assert.equal(count, 1);
    const bytes = await f.maintenance.backup(), preview = await f.maintenance.preview(bytes, f.login.user.id);
    await f.maintenance.exclusive(async () => await f.maintenance.replace('restore', preview.token, f.login.user.id));
    const restored = (await f.runtime.workspaces.open('asan-ax')).db;
    assert.equal((await (restored.prepare('SELECT COUNT(*) n FROM submissions')).get()).n, 1);
    assert.equal((await (restored.prepare('SELECT COUNT(*) n FROM lms_outbox')).get()).n, count);
    assert.equal((await (restored.prepare('SELECT state FROM lms_outbox')).get()).state, 'held');
});
