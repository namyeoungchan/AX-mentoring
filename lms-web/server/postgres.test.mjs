...
pgTest('PostgreSQL attendance handles concurrent students and duplicate codes without overwriting corrections',async t=>{
  ...
  const admin=f.login.user,codes=createAttendanceCodes(db,r.workspaces,r.attendance,{now:()=>Date.parse('2026-09-18T01:00:00Z')})
  ...
  const {code}=await codes.issue('default',{...selection,startTime:'10:00',endTime:'12:00'},admin)
  ...
  assert.equal((await db.prepare("SELECT COUNT(*) AS n FROM lms_records WHERE kind='attendancePresence'").get()).n,20)
