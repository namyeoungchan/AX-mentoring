...
export function studentLearning(db, user, verified = false) {
  ...
  const confirmed = row => {
    const round = db.prepare('SELECT state FROM lms_attendance_rounds WHERE course_id=? AND date=? AND period=?').get(row.courseId, row.date, row.period)
  ...
  return {
    ...
    attendance: course ? records('attendance').filter(row => row.studentId === learner.id && row.courseId === course.id && confirmed(row)).map(row => ({ id: row.id, date: row.date, period: row.period, status: row.status })) : [],
