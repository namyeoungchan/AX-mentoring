...
export async function initializeVerification(db) {
    await db.exec(`CREATE TABLE IF NOT EXISTS lms_workspace_verifications (
...
export async function workspaceForGuild(db, guildId) {
    if (!await tableExists(db, 'lms_workspace_guilds'))
        return null;
    return (await (db.prepare('SELECT workspace_id FROM lms_workspace_guilds WHERE guild_id=?')).get(guildId))?.workspace_id || null;
}
export async function workspaceVerified(db, workspaceId, userId, guildId) {
    return Boolean(await (db.prepare(`SELECT 1 FROM lms_workspace_verifications v
    ...
    WHERE v.workspace_id=? AND v.user_id=? AND (? IS NULL OR v.guild_id=?)`)).get(workspaceId, userId, guildId || null, guildId || null));
}
...
export async function migrateVerification(db) {
    await initializeVerification(db);
    if (await (db.prepare("SELECT 1 FROM lms_workspace_migrations WHERE name='verification-v1'")).get())
        return;
    await (db.prepare(`INSERT OR IGNORE INTO lms_workspace_verifications
    ...
    if (await tableExists(db, 'lms_staff_connections'))
        await (db.prepare(`INSERT OR IGNORE INTO lms_workspace_verifications
    ...
    WHERE u.verified_at IS NOT NULL AND length(u.discord_id) BETWEEN 17 AND 20
      AND u.discord_id NOT GLOB '*[^0-9]*'`)).run();
    if (await tableExists(db, 'lms_admissions'))
        await (db.prepare(`INSERT OR IGNORE INTO lms_workspace_verifications
    ...
    await (db.prepare('UPDATE lms_registrations SET expires_at=0 WHERE workspace_id IS NULL AND verified_at IS NULL')).run();
    await (db.prepare("INSERT INTO lms_workspace_migrations VALUES('verification-v1')")).run();
}
