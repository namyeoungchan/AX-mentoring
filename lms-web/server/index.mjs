...
app.use('/api', (req, res, next) => {
  ...
  if (maintenance.busy && req.path !== '/health') return res.status(503).json({ error: '데이터 관리 작업 중입니다. 잠시 후 다시 시도하세요.' })
  next()
})
...
app.use('/api/admin/data', (req, res, next) => {
  if (maintenance.busy) return res.status(503).json({ error: '데이터 관리 작업 중입니다. 잠시 후 다시 시도하세요.' })
  try {
    const user = auth.session(sessionToken(req))
    if (!user) return res.status(401).json({ error: '로그인이 필요합니다.' })
    auth.platformAdmin(user)
    next()
...
app.get('/api/health', (_req, res) => maintenance.broken ? res.status(503).json({ status: 'recovery-required' }) : res.json({ status: 'ok', database: 'connected', auth: 'required' }))
...
app.use('/api/admin/data', (req, _res, next) => { auth.platformAdmin(req.account); next() })
app.get('/api/admin/data', (_req, res) => res.json(maintenance.status()))
const sendBackup = (res, bytes, name) => res.type('application/octet-stream').attachment(name).send(bytes)
app.get('/api/admin/data/backup', async (req, res) => {
  const bytes = await maintenance.exclusive(() => {
    auth.platformAdmin(auth.session(sessionToken(req)))
    return maintenance.backup()
  })
  sendBackup(res, bytes, `learningops-${new Date().toISOString().slice(0, 10)}.axbackup`)
})
app.get('/api/admin/data/backups/:id', (req, res) => sendBackup(res, maintenance.download(req.params.id), req.params.id))
app.post('/api/admin/data/preview', async (req, res) => {
  auth.limit('backup-preview', req.account.id, 10, 15 * 60000)
  const encoded = req.body?.backup
  if (typeof encoded !== 'string' || encoded.length > Math.ceil(MAX_BACKUP_BYTES / 3) * 4) throw new ApiError(413, '백업 파일은 최대 32MB까지 지원합니다.')
  const bytes = Buffer.from(encoded, 'base64')
  if (bytes.toString('base64') !== encoded) throw new ApiError(422, '백업 파일 형식이 올바르지 않습니다.')
  res.json(await maintenance.exclusive(() => {
    auth.platformAdmin(auth.session(sessionToken(req)))
    return maintenance.preview(bytes, req.account.id)
  }))
})
for (const action of ['reset', 'restore']) app.post(`/api/admin/data/${action}`, async (req, res) => {
  auth.limit('data-replace', req.account.id, 5, 15 * 60000)
  if (req.body?.confirmation !== (action === 'reset' ? '전체 데이터 초기화' : '백업으로 전체 복구')) throw new ApiError(422, '확인 문구를 정확히 입력하세요.')
  const result = await maintenance.exclusive(async () => {
    const user = auth.session(sessionToken(req))
    await auth.confirmAdmin(user, req.body?.currentPassword)
    return maintenance.replace(action, req.body?.token, user.id)
  })
...
auth.cleanup()
const cleanup = setInterval(() => { if (!maintenance.busy) auth.cleanup() }, 60000).unref()
