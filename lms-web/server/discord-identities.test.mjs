...
test('Discord authentication cleanup releases ownership, protects scope and supports re-verification', async t => {
    ...
    const { owner, discordId, guildId, first, workspace } = await discordIdentitiesScenario(runtime);
    ...
    await runtime.store.db.exec("CREATE TRIGGER fail_identity_audit BEFORE INSERT ON lms_account_audit WHEN NEW.action LIKE 'discord.identity.clear%' BEGIN SELECT RAISE(ABORT,'audit unavailable'); END;");
    ...
    const scoped = (await runtime.discordIdentities.list(owner,workspace.id)).identities.find(row=>row.discordId===discordId);
    await assert.rejects(runtime.discordIdentities.remove(owner,discordId,{discordId,revision:scoped.revision},workspace.id),/audit unavailable/);
    assert.equal((await runtime.auth.verificationState({discordId,guildId})).verified,true);
    assert.ok(await runtime.auth.session(login.token));
    assert.equal((await runtime.discordIdentities.list(owner,workspace.id)).identities.find(row=>row.discordId===discordId).revision,scoped.revision);
});
