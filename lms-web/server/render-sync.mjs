...
export function createRenderSync(db, { token = '', sourceId = 'asan-ax', now = Date.now } = {}) {
  if (token && token.length < 32) throw new Error('LEARNINGOPS_SYNC_TOKEN must contain at least 32 characters')
  db.exec('CREATE TABLE IF NOT EXISTS lms_remote_snapshots (source_id TEXT PRIMARY KEY, received_at TEXT NOT NULL, payload TEXT NOT NULL)')
  ...
  function ingest(raw) {
    ...
    if (payload.sourceId !== sourceId) throw Object.assign(new Error('등록된 데이터 소스가 아닙니다.'), { status: 403 })
    const timestamp = Date.parse(payload.capturedAt)
    if (Math.abs(timestamp - now()) > 5 * 60000) throw Object.assign(new Error('동기화 시각이 유효하지 않습니다.'), { status: 422 })
    for (const key of ['mentors', 'bookings', 'assignments', 'submissions']) {
      if (payload[key].length !== Math.min(payload.counts[key], payload.rowLimit) || new Set(payload[key].map(row => row.id)).size !== payload[key].length) throw Object.assign(new Error('데이터 건수가 일치하지 않습니다.'), { status: 422 })
    }
    const previous = db.prepare('SELECT payload FROM lms_remote_snapshots WHERE source_id=?').get(sourceId)
    if (previous && Date.parse(JSON.parse(previous.payload).capturedAt) >= timestamp) throw Object.assign(new Error('이미 수신했거나 이전에 생성된 데이터입니다.'), { status: 409 })
    db.prepare('INSERT INTO lms_remote_snapshots VALUES(?,?,?) ON CONFLICT(source_id) DO UPDATE SET received_at=excluded.received_at,payload=excluded.payload').run(sourceId, new Date(now()).toISOString(), JSON.stringify(payload))
    return { ok: true }
  }
  function read() {
    const row = db.prepare('SELECT * FROM lms_remote_snapshots WHERE source_id=?').get(sourceId)
    if (!row) return { configured: Boolean(token), sourceId, state: 'waiting', snapshot: null, receivedAt: null }
