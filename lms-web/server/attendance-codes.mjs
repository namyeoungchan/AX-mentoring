...
export function createAttendanceCodes(identityDb, workspaces, attendance, { now = Date.now, enabled = true } = {}) {
    ...
    async function issue(id, raw, user) {
        if (!enabled)
            throw new ApiError(409, 'Discord 인증 봇 연결을 설정하세요.');
        const input = issueInput.parse(raw), { roster, meta, db } = await context(id, input, user);
        // Issuers must be durable accounts so revoking their role also invalidates their codes.
        if (!await (identityDb.prepare('SELECT 1 FROM lms_users WHERE id=?')).get(user.id))
            throw new ApiError(403, '개인 관리자 또는 멘토 계정으로 로그인하세요.');
        if (meta.guildIds.length !== 1)
            throw new ApiError(409, 'Discord 서버 하나를 연결하세요.');
        if (roster.state !== '진행 중')
            throw new ApiError(409, '관리자 또는 메인 강사가 회차를 시작한 뒤 코드를 발급하세요.');
        const students = roster.rows.filter(r => r.enrollment === '정상').map(r => r.studentId);
        if (!students.length)
            throw new ApiError(422, '코드를 사용할 정상 수강생이 없습니다.');
        ...
        try {
            ...
            for (let attempts = 0; attempts < 30; attempts++) {
                ...
                if (!await (db.prepare('SELECT 1 FROM lms_attendance_codes WHERE code_hash=?')).get(digest(candidate))) {
                    code = candidate;
                    break;
                }
            }
            if (!code)
                throw new ApiError(503, '코드를 생성하지 못했습니다. 다시 시도하세요.');
            ...
            await (db.prepare('INSERT INTO lms_audit(actor,action,target,after_json) VALUES(?,?,?,?)')).run(user.username, 'attendance.code.issue', codeId, JSON.stringify({ ...input, expiresAt, students: students.length }));
    ...
    async function checkIn(raw) {
        const input = checkInput.parse(raw), id = await workspaceForGuild(identityDb, input.guildId);
        if (!id || (await workspaces.metadata(id)).archivedAt !== null)
            throw new ApiError(403, '이 서버의 출석 등록을 사용할 수 없습니다.');
        const account = await (identityDb.prepare('SELECT id,username,platform_role AS role FROM lms_users WHERE discord_id=?')).get(input.discordId);
        if (!account || await workspaces.role(id, account) !== 'student' || !await workspaceVerified(identityDb, id, account.id, input.guildId))
            throw new ApiError(403, '이 서버에서 LMS 학생 계정의 Discord 인증을 완료하세요.');
        const db = (await workspaces.open(id)).db;
        await db.exec('BEGIN IMMEDIATE');
        try {
            const code = await (db.prepare('SELECT * FROM lms_attendance_codes WHERE code_hash=? AND guild_id=?')).get(digest(input.code), input.guildId);
            if (!code || code.revoked_at !== null || code.expires_at <= now())
                throw new ApiError(410, '만료되었거나 사용할 수 없는 출석 코드입니다. 멘토에게 새 코드를 확인하세요.');
            ...
            if (!issuer)
                throw new ApiError(410, '코드 발급자의 권한이 변경되었습니다. 새 코드를 발급받으세요.');
            const roster = await attendance.view(id, selected, issuer);
            if (roster.state !== '진행 중')
                throw new ApiError(409, '출석 등록이 마감된 회차입니다.');
            const learner = await (db.prepare("SELECT id,data FROM lms_records WHERE kind='learners' AND json_extract(data,'$.discordId')=?")).get(input.discordId);
            if (!learner || !JSON.parse(code.student_ids).includes(learner.id) || !roster.rows.some(r => r.studentId === learner.id && r.enrollment === '정상'))
                throw new ApiError(403, '이 코드의 출석 대상 수강생이 아닙니다.');
            const existing = await (db.prepare("SELECT data FROM lms_records WHERE kind='attendance' AND json_extract(data,'$.studentId')=? AND json_extract(data,'$.courseId')=? AND json_extract(data,'$.date')=? AND json_extract(data,'$.period')=?")).get(learner.id, code.course_id, code.date, code.period);
            if (existing) {
                await db.exec('COMMIT');
            ...
            await (db.prepare("INSERT INTO lms_records(kind,id,data) VALUES('attendance',?,?)")).run(record.id, JSON.stringify(record));
            await (db.prepare('INSERT INTO lms_audit(actor,action,target,after_json) VALUES(?,?,?,?)')).run(account.username, 'attendance.checkin', record.id, JSON.stringify(record));
            await (db.prepare('UPDATE lms_attendance_rounds SET version=version+1 WHERE course_id=? AND date=? AND period=?')).run(code.course_id, code.date, code.period);
            await db.exec('COMMIT');
