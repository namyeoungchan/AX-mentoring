...
const publicUser = row => ({ id: row.id, username: row.username, name: row.name, discordId: row.discord_id, role: 'student' })
...
export function createAuth(db, { adminPassword = '', botToken = '', guildId = '', now = Date.now } = {}) {
  ...
  function renew(ticket) {
    ...
    available(row.username, row.discord_id)
    const value = challenge()
    db.prepare('UPDATE lms_registrations SET code_hash=?,expires_at=?,renewed_at=?,guild_id=? WHERE ticket_hash=?').run(hash(value.raw), value.expiresAt, now(), guildId, row.ticket_hash)
    return { code: value.code, expiresAt: value.expiresAt }
  }
  function botAuthorized(header = '') { return enabled && safeEqual(header, `Bearer ${botToken}`) }
  function verificationRequest(body) {
    requireEnabled()
    const input = verification.parse(body)
    if (input.guildId !== guildId) throw new ApiError(403, '인증이 허용된 Discord 서버가 아닙니다.')
  ...
  function verify(body) {
    ...
    try {
      ...
      db.prepare('INSERT INTO lms_users(id,username,name,password_hash,discord_id,guild_id,created_at) VALUES(?,?,?,?,?,?,?)').run(randomUUID(), row.username, row.name, row.password_hash, row.discord_id, row.guild_id, now())
  ...
  return { enabled, register, status, renew, botAuthorized, preview, verify, login, adminLogin, session, logout, limit, cleanup }
}
