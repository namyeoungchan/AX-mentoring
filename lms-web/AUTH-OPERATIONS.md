...
최초 관리자 등록 여부와 관계없이 실제 운영 DB를 사용하는 서버 터미널에서 실행합니다.

```sh
cd /app/lms-web
npm run account:super -- moco.admin
```

비밀번호는 화면에 표시되지 않는 입력으로 두 번 받으며 소스코드·명령 인자·환경변수·로그에 저장하지 않습니다. DB에는 무작위 salt를 적용한 scrypt 해시만 저장합니다. 서버의 `DATABASE_URL`(PostgreSQL) 또는 `BOT_DB_PATH`(SQLite)를 사용하며, SQLite 파일이 없으면 잘못된 DB 생성을 막기 위해 중단합니다. 같은 아이디가 이미 있으면 덮어쓰거나 승격하지 않습니다.

슈퍼계정은 모든 워크스페이스의 총관리 권한을 가지며 별도 `is_super_admin` 값으로 구분합니다. 웹에서는 슈퍼계정 삭제·초기 비밀번호 발급이 불가능하고, 본인 비밀번호 변경과 서버의 `account:reset` 명령을 통한 복구는 가능합니다. 공개 가입·최초 관리자 등록 API로 슈퍼계정을 만들 수 없습니다. 슈퍼계정 생성 후에는 최초 관리자 등록과 개발용 공용 로그인이 닫힙니다. 생성 시 기존 개인 관리자 계정과 비밀번호는 유지됩니다.
