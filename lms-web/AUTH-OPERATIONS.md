...
**역할 변경**은 현재 선택한 워크스페이스에 적용됩니다. 총관리자는 일반 계정에 워크스페이스 관리자·메인 강사·조 담당 멘토·수강생 역할을 지정할 수 있습니다. 조 담당 멘토는 담당 조를 하나 이상 선택해야 합니다. 아직 소속이 없는 계정에도 현재 워크스페이스 역할을 지정할 수 있습니다. 총관리자·슈퍼계정 자체의 권한은 이 기능으로 변경하지 않습니다.

변경 시 대상 계정의 기존 로그인을 해제합니다. 테스트 계정으로 다시 로그인하면 선택한 역할의 화면이 표시됩니다. 다른 워크스페이스의 역할, 비밀번호, Discord 인증과 학습 이력은 유지됩니다. 수강생 학습 콘텐츠에는 별도의 과정·조 배정과 Discord 인증이 필요합니다. 변경 기록은 `lms_audit`의 `account.workspace-role`에 남습니다.
